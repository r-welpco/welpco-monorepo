...
### 2b. Certn webhook accepts unsigned payloads when the secret is unset — ✅ done (2026-07-10)
The safety-verification Certn webhook verifies HMAC signatures only if `CERTN_WEBHOOK_SECRET` is configured — otherwise it **accepted the payload with just a warning log**. A misconfigured production environment would let anyone forge background-check results. **Fixed:** `certn-webhook.controller.ts` now fails closed — missing secret under `NODE_ENV=production` rejects with 503 before any processing; the warn-and-accept path remains for non-production only. Invalid/missing signatures were already rejected (400) whenever the secret is set. Covered by `certn-webhook.controller.spec.ts` (missing-secret-in-prod rejected, missing-secret-in-dev accepted with warning, bad/missing signature rejected, valid signature on both header variants accepted).

### 2d. Login endpoint has no request-rate cap — ✅ done (2026-07-10)
The per-email login rate limit formerly at the auth controller was removed (`apps/bff/src/modules/auth/auth.controller.ts:56-59`) — brute-force protection relied solely on `AccountLockoutService`. Lockout throttles per-account guessing but not credential-stuffing sweeps across many accounts or resource exhaustion on the bcrypt-verify path. **Fixed (LOGIN-003):** `POST /auth/login` now carries `RateLimitGuard` + `@RateLimit({ ttl: 900, limit: 10 })` keyed per email (IP fallback) — deliberately wider than the 5-attempt lockout so legitimate users hit the friendlier lockout message first. Guard returns 429 when exceeded and is bypassed under `NODE_ENV=test` / `DISABLE_RATE_LIMIT=true`, so e2e/seed flows (and signup's single post-signup login) are unaffected.
