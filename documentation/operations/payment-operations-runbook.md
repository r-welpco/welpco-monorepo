...
> Last verified: 2026-07-03 · commit de88bd4 (includes the 2026-07-03 payout changes: `approved` batch status support, Friday-index migration `20260703000001`, recovery/tax admin endpoints) · generated from implementation
...
| Eligibility rules (Friday, 7-day hold) | `apps/bff/src/domains/payment/payout-eligibility.ts` |
...
| `approved` | **Reserved.** In the enum and treated as "active" by the unique Friday index and by the existing-batch queries, but no code path currently sets it — approval and execution are a single step | — (no setter in code today) |
...
2. **`POST /api/admin/payouts/batches/build`** → `PayoutBatchService.buildDraftBatch(friday)`:
   - Resets retryable `failed` lines (no `stripe_transfer_id`) back to `pending`.
   - If a `review` batch already exists for that Friday it is deleted and rebuilt (its `scheduled` lines return to `pending` first). If the existing batch is `approved` or `executing`, the build is rejected (`400: already <status> and cannot be rebuilt`).
   - Selects eligible lines (`pending`/`failed`, no transfer id, `welper_net_cents > 0`, not `stripe_fee_pending`, booking is `payment_released`/`completed` and not `disputed`, and payment released ≥ 7 days before the payout Friday — `PAYOUT_HOLD_DAYS = 7`, timezone `America/Toronto`).
   - Creates the batch as `review`; lines become `scheduled`.
   - Only the upcoming Friday or a past Friday may be built (`assertBuildablePayoutFriday`).
3. **`POST /api/admin/payouts/batches/:id/approve`** → `PayoutBatchService.approveAndExecute(batchId, adminUserId)`:
   - Rejects unless batch is `review` and today ≥ payout Friday (Toronto calendar day, `isPayoutFridayReached`).
...
## The Friday active-batch unique index
...
Operational implication: **at most one active batch per payout Friday** across `review`/`approved`/`executing`. The database now also blocks a second batch while one is `approved`, matching the service queries (`payout-batch.service.ts:130,254`) that already treated `approved` as active. Terminal batches (`completed`, `partial`, `failed`) never block a rebuild — but `buildDraftBatch` only ever rebuilds `review` batches; after a `partial`/`failed` run, retryable lines flow into the **next** Friday's batch instead.
...
|---|---|---|---|
| `GET /payouts/upcoming` | `getUpcomingPreview()` | Preview of upcoming Friday: eligible pending lines, or the existing active batch | Weekly review before building |
| `POST /payouts/batches/build` (body: optional `payoutFriday` YYYY-MM-DD) | `buildDraftBatch()` | Build or rebuild the `review` batch for a Friday | Before approval; after fixing excluded/failed lines |
| `GET /payouts/batches?payoutFriday=&limit=` | `listBatches()` | Batch summaries (max 100) | History / find a batch id |
...
| `POST /payouts/batches/:id/approve` | `approveAndExecute()` | Approve **and execute** Stripe transfers (single step) | On/after payout Friday, after review checks pass |
...
- **Retry path.** `failed` lines with `stripe_transfer_id IS NULL` are automatically reset to `pending` by the next `buildDraftBatch` and picked up by the next Friday's batch. **Never** retry a line that already has a transfer id — the code refuses to (`Ledger line ... already has a Stripe transfer`), and so should you.
...
|---|---|---|
| `stripe.transfers.create` | `payout-batch.service.ts` (`transferWelperLinesAtomically`) | CAD, `transfer_group` = batch id, metadata `batchId`/`welperId`/`payoutFriday`; E2E accounts (prefix `E2E_STRIPE_CONNECT_ACCOUNT_PREFIX`) short-circuit to fake ids outside production |
...
|---|---|
| Batch stuck in `review`, approve returns 400 | Is it payout Friday yet (Toronto)? `Transfers are only allowed on or after payout Friday`. Connect-not-ready welpers? `GET /payouts/batches/:id` shows `connectReady` per welper — have the welper finish Stripe onboarding, or rebuild without them (their lines drop out only if excluded; otherwise approval stays blocked). |
...
| Duplicate active batch error on build | The unique index (`IDX_payout_batches_active_friday`) fired: a `review`/`approved`/`executing` batch already exists for that Friday. `SELECT id, status FROM payout_batches WHERE payout_friday = '...';` |
