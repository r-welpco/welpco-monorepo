...
- `create()` — transactional: locks the booking, verifies participant + `validateTransition(status, DISPUTED)` + report window open (`isDisputeReportWindowOpen`, default 10 min after completion, setting `dispute_report_window_minutes`), rejects a second open dispute, validates evidence ownership (files by S3 key prefix, messages must belong to the booking's chat thread), saves the dispute `open`, sets the booking `disputed`, and calls `WelperPayoutLedgerService.excludeForDispute()` (ledger line → `excluded`/`dispute_open`, detached from any scheduled payout batch, batch totals recalculated). Notifies the counterparty.
