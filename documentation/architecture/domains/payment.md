...
- **Authorization hold** — `prepareAuthorizationForAcceptance(bookingId)`: called from `BookingService.accept()`. Computes due/deadline windows from the scheduled start (authorize 72 h before start; hard deadline 24 h before). If the due time is in the future the hold is `scheduled`; otherwise `authorizeHoldBeforeWelperAccept()` runs immediately: manual-capture, off-session PaymentIntent for one hour of service + tax. Each Stripe attempt keeps its own payment row and records Charge/card/`capture_before` evidence. A hold must remain valid through scheduled end plus the configured buffer; missing expiry is flagged for review. SCA cards fail with code `payment_requires_action`.
- **Deferred processing** — `processDeferredAuthorizations()` (lease-locked, `skip_locked`) authorizes due scheduled holds. Automatic Stripe expiry can schedule one replacement before the cutoff. `cancelExpiredAuthorizationBookings()` cancels unresolved or unsafe accepted bookings fee-free at the deadline.
...
- **Cancellation** — `onBookingCanceled()`: cancels uncaptured PaymentIntents. A one-hour late fee is captured only for an explicit customer cancellation inside the free-cancellation window and only after Stripe confirms the hold is still capturable; system/payment-deadline cancellations cannot charge a fee.
...
Timing rules in `payout-eligibility.ts`: `PAYOUT_HOLD_HOURS = 48`. A ledger line becomes eligible after 48 complete elapsed hours from `paymentReleasedAt`; this is an instant-based duration, not two calendar dates. New batches may only be built for the upcoming Monday or an earlier Monday (`assertBuildablePayoutDate`). Transfers may execute on or after the stored date (`isPayoutDateReached`), including historical Friday batches created before the weekday change.
...
3. **Build** — `buildDraftBatch(payoutDate?)` (transactional): resets retryable `failed` lines to `pending`; if an active batch exists for that date it must be in `review` (its `scheduled` lines are released and the batch deleted) — a batch in `approved`/`executing` throws "already {status} and cannot be rebuilt". Eligible lines (status `pending`/`failed`, no transfer id, net > 0, 48-hour hold met, booking in `payment_released`/`completed` and not `disputed`) are locked, marked `scheduled`, and attached to a new `review` batch with computed totals (`payout-batch-totals.util.ts`). The legacy-named partial unique index `IDX_payout_batches_active_friday` guarantees at most one active batch per payout date.
...
5. **Approve + execute** — `approveAndExecute(batchId, adminUserId)` (admin endpoint `POST /api/admin/payouts/batches/:id/approve`):
   ...
   - Transactionally locks the batch and its `scheduled` lines, re-validates each line (net > 0, no existing transfer, 48-hour hold, booking not disputed/missing), then sets `status = executing`, `approvedBy`, `approvedAt`.
